...
import { handleMeetingChatSend } from "./handlers/meeting-chat.js";
  ...
  void (async () => {
    try {
      ...
      if (message.type.startsWith("lobby.")) {
        switch (message.type) {
          case "lobby.admit":
            await handleLobbyAdmit(ctx, message);
            break;
          case "lobby.deny":
            await handleLobbyDeny(ctx, message);
            break;
          case "lobby.list":
            handleLobbyList(ctx, message);
            break;
          default:
            sendMessage(ctx.send, {
              type: "error",
              code: "unknown_type",
              message: "Unsupported lobby message type",
              v: SIGNALING_VERSION,
            });
        }
        return;
      ...
      if (message.type === "meeting.chat.send") {
        handleMeetingChatSend(ctx, message);
        return;
      }
